...
def test_runtime_dictionary_includes_cmudict_only_words(
    curated_lexicon_path: Path,
    cmudict_path: Path,
) -> None:
    service = LexiconService(curated_lexicon_path, cmudict_path=cmudict_path)

    assert "thought TH AO1 T" in service.runtime_dictionary_lines()
    assert "work W ER1 K" in service.runtime_dictionary_lines()
