...
```bash
bash scripts/benchmark_remote_gpu.sh
bash scripts/benchmark_mfa_cli.sh
python -m pronunciation_backend.benchmark --word work --repeat 10 --json
```

If a real clip is available, pass it as the first script argument or via `--audio`. Without one, the benchmark tools generate a synthetic WAV and default to `work`, which exercises CMUdict-backed lookup outside the curated reference lexicon.
